...
<?php
try {
    // Получаем данные из всех трех таблиц
    $stroit_akt = $db->query("SELECT * FROM stroit_akt")->fetchAll(PDO::FETCH_ASSOC);
    $stroit_foto = $db->query("SELECT * FROM stroit_foto")->fetchAll(PDO::FETCH_ASSOC);
    $stroit_liter = $db->query("SELECT * FROM stroit_liter")->fetchAll(PDO::FETCH_ASSOC);
...
$all_data = [
    'stroit_akt' => $stroit_akt,
    'stroit_foto' => $stroit_foto,
    'stroit_liter' => $stroit_liter
];
?>
...
<head>
    ...
    <title>История строительства корпуса - Виртуальный музей</title>
...
<body>
    ...
    <header class="page-header">
        <div class="container">
            <h1>История строительства корпуса</h1>
            <p class="lead">Архивные материалы, фотографии и документы</p>
        </div>
    </header>
    ...
    <main class="container mb-5">
        ...
        foreach ($all_data as $tableName => $data):
            if (!empty($data)): 
                // Определяем категорию для бейджа
                $category = '';
                if ($tableName === 'stroit_akt') $category = 'Акты и документы';
                elseif ($tableName === 'stroit_foto') $category = 'Фотографии';
                elseif ($tableName === 'stroit_liter') $category = 'Литература';
                ?>
                    ...
                    <div class="modal-dialog modal-dialog-centered modal-xl">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title"><?= htmlspecialchars($data[0]['Nazvanie'] ?? 'Без названия'); ?></h5>
                                    ...
                                    <div class="carousel-inner">
                                        <?php foreach ($data as $key => $item): ?>
                                            <div class="carousel-item <?= $key === 0 ? 'active' : ''; ?>">
                                                <img src="<?= htmlspecialchars($item['Img'] ?? 'placeholder.jpg'); ?>" class="d-block w-100" alt="<?= htmlspecialchars($item['Nazvanie'] ?? ''); ?>">
                    ...
                    <?php if ($carousel_index % 2 == 0): ?>
                        <div class="collection-image">
                            ...
                            <a href="#" data-bs-toggle="modal" data-bs-target="#carouselModal-<?= $carousel_index; ?>">
                                <img src="<?= htmlspecialchars($data[0]['Img'] ?? 'placeholder.jpg'); ?>" alt="<?= htmlspecialchars($data[0]['Nazvanie'] ?? ''); ?>">
                            </a>
                        </div>
                        <div class="collection-text">
                            ...
                            <h2>
                                <a href="#" data-bs-toggle="modal" data-bs-target="#carouselModal-<?= $carousel_index; ?>">
                                    <?= htmlspecialchars($data[0]['Nazvanie'] ?? 'Без названия'); ?>
                                </a>
                            </h2>
                            <p><?= htmlspecialchars($data[0]['Opisanie'] ?? $data[0]['Text'] ?? 'Описание отсутствует'); ?></p>
                            <button class="gallery-btn" data-bs-toggle="modal" data-bs-target="#carouselModal-<?= $carousel_index; ?>">
                                <i class="bi bi-images"></i> Смотреть материалы
                            </button>
                        </div>
                    <?php else: ?>
                        <div class="collection-text">
