...
<?php
try {
    $sport_izvest = $db->query("SELECT * FROM sport_izvest")->fetchAll(PDO::FETCH_ASSOC);
    $sport_str_istor = $db->query("SELECT * FROM sport_str_istor")->fetchAll(PDO::FETCH_ASSOC);
    $sport_yspex = $db->query("SELECT * FROM sport_yspex")->fetchAll(PDO::FETCH_ASSOC);
...
$all_data = [
    'sport_izvest' => $sport_izvest,
    'sport_str_istor' => $sport_str_istor,
    'sport_yspex' => $sport_yspex
];
?>
...
<head>
    ...
    <title>Спортивные достижения - Виртуальный музей</title>
...
<body>
    ...
    <header class="page-header">
        <div class="container">
            ...
            <p class="lead">История побед и выдающихся спортсменов нашего учебного заведения</p>
        </div>
    </header>
