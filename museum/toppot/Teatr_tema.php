...
<?php
try {
    $teatr_kollek = $db->query("SELECT * FROM teatr_kollekz")->fetchAll(PDO::FETCH_ASSOC);
    $teatr_nashi_vipusk = $db->query("SELECT * FROM teatr_nashi_vipusk")->fetchAll(PDO::FETCH_ASSOC);
...
$all_data = [
    'teatr_kollekz' => $teatr_kollek,
    'teatr_nashi_vipusk' => $teatr_nashi_vipusk
];
?>
...
<head>
    ...
    <title>Театральные коллекции - Виртуальный музей</title>
...
<body>
    ...
    <header class="page-header">
        <div class="container">
            <h1>Театральные коллекции</h1>
            <p class="lead">История театральных постановок и выпускников театрального отделения</p>
        </div>
    </header>
