...
<?php
try {
    $voina_istor = $db->query("SELECT * FROM voina_istor")->fetchAll(PDO::FETCH_ASSOC);
    $voina_sotrudniki = $db->query("SELECT * FROM voina_sotrudniki")->fetchAll(PDO::FETCH_ASSOC);
    $voina_SVO = $db->query("SELECT * FROM voina_SVO")->fetchAll(PDO::FETCH_ASSOC);
...
$all_data = [
    'voina_istor' => $voina_istor,
    'voina_sotrudniki' => $voina_sotrudniki,
    'voina_SVO' => $voina_SVO
];
?>
...
<head>
    ...
    <title>Военная история - Виртуальный музей</title>
...
<body>
    ...
    <header class="page-header">
        <div class="container">
            <h1>Военная история</h1>
            <p class="lead">Подвиги сотрудников и выпускников в военных конфликтах</p>
        </div>
    </header>
