...
<?php
try {
    $znamenitosti = $db->query("SELECT * FROM znamenitosti")->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    ...
    <title>Знаменитости - Виртуальный музей</title>
...
<body>
    ...
    <header class="page-header">
        <div class="container">
            <h1>Известные выпускники</h1>
            <p class="lead">Знаменитые личности, связанные с нашей историей</p>
        </div>
    </header>
    ...
    <main class="container mb-5">
        ...
        <?php
        $carousel_index = 0; 
        foreach ($all_data['znamenitosti'] as $key => $person): 
            if (!empty($person)): ?>
                <!-- Модальное окно галереи -->
                    ...
                    <div class="modal-dialog modal-dialog-centered modal-xl">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title"><?= htmlspecialchars($person['FIO'] ?? 'Без имени'); ?></h5>
                                ...
                                <div id="carousel-<?= $carousel_index; ?>" class="carousel slide" data-bs-ride="carousel">
                                    <div class="carousel-inner">
                                        <div class="carousel-item active">
                                            <img src="<?= htmlspecialchars($person['Img'] ?? 'placeholder.jpg'); ?>" class="d-block w-100" alt="<?= htmlspecialchars($person['FIO'] ?? ''); ?>">
                                            <div class="carousel-caption d-none d-md-block">
                                                <p><?= htmlspecialchars($person['FIO'] ?? ''); ?></p>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <?php if ($key % 2 == 0): ?>
                        <div class="collection-image">
                            ...
                            <a href="#" data-bs-toggle="modal" data-bs-target="#carouselModal-<?= $carousel_index; ?>">
                                <img src="<?= htmlspecialchars($person['Img'] ?? 'placeholder.jpg'); ?>" alt="<?= htmlspecialchars($person['FIO'] ?? ''); ?>">
                            </a>
                        </div>
                        <div class="collection-text">
                            <h2>
                                <a href="#" data-bs-toggle="modal" data-bs-target="#carouselModal-<?= $carousel_index; ?>">
                                    <?= htmlspecialchars($person['FIO'] ?? 'Без имени'); ?>
                                </a>
                            </h2>
                            <?php if (!empty($person['Gody_zhizni'])): ?>
                                <span class="person-years"><?= htmlspecialchars($person['Gody_zhizni']); ?></span>
                            <?php endif; ?>
                            <p><?= htmlspecialchars($person['Opisanie'] ?? $person['Dostizheniya'] ?? 'Описание отсутствует'); ?></p>
                            <button class="gallery-btn" data-bs-toggle="modal" data-bs-target="#carouselModal-<?= $carousel_index; ?>">
                                <i class="bi bi-images"></i> Подробнее
                            </button>
                        </div>
                    <?php else: ?>
                        <div class="collection-text">
