...
<?php
try {
    $meropriyatiya = $db->query("SELECT * FROM meropriyatiya")->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    ...
    <title>Мероприятия - Виртуальный музей</title>
...
<body>
    ...
    <header class="page-header">
        <div class="container">
            <h1>Мероприятия музея</h1>
            <p class="lead">Актуальные события и предстоящие выставки</p>
        </div>
    </header>
    ...
    <main class="container mb-5">
        ...
        <?php
        $carousel_index = 0; 
        foreach ($all_data['meropriyatiya'] as $key => $event): 
            if (!empty($event)): ?>
                <!-- Модальное окно галереи -->
                    ...
                    <div class="modal-dialog modal-dialog-centered modal-xl">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title"><?= htmlspecialchars($event['Nazvanie']); ?></h5>
                                ...
                                <div id="carousel-<?= $carousel_index; ?>" class="carousel slide" data-bs-ride="carousel">
                                    <div class="carousel-inner">
                                        <div class="carousel-item active">
                                            <img src="<?= htmlspecialchars($event['Img']); ?>" class="d-block w-100" alt="<?= htmlspecialchars($event['Nazvanie']); ?>">
                                            <div class="carousel-caption d-none d-md-block">
                                                <p><?= htmlspecialchars($event['Nazvanie']); ?></p>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <?php if ($key % 2 == 0): ?>
                        <div class="collection-image">
                            ...
                            <a href="#" data-bs-toggle="modal" data-bs-target="#carouselModal-<?= $carousel_index; ?>">
                                <img src="<?= htmlspecialchars($event['Img']); ?>" alt="<?= htmlspecialchars($event['Nazvanie']); ?>">
                            </a>
                        </div>
                        <div class="collection-text">
                            <h2>
                                <a href="#" data-bs-toggle="modal" data-bs-target="#carouselModal-<?= $carousel_index; ?>">
                                    <?= htmlspecialchars($event['Nazvanie']); ?>
                                </a>
                            </h2>
                            <?php if (!empty($event['Data'])): ?>
                                <div class="event-date">
                                    <i class="bi bi-calendar"></i> <?= htmlspecialchars($event['Data']); ?>
                                </div>
                            <?php endif; ?>
                            <p><?= htmlspecialchars($event['Opisanie']); ?></p>
                            <button class="gallery-btn" data-bs-toggle="modal" data-bs-target="#carouselModal-<?= $carousel_index; ?>">
                                <i class="bi bi-images"></i> Подробнее о мероприятии
                            </button>
                        </div>
                    <?php else: ?>
                        <div class="collection-text">
